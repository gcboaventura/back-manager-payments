...
export class UpdateItemPlanMysql implements UpdateItemPlanRepository {
	...
	async update(item: ItemModelPlan): Promise<ResponseHandleItemsPlans> {
		return new Promise((resolve, reject) => {
			this.connection.query(
				`UPDATE ITEMS_PLAN SET name = ?, description = ?, quantity = ?, price = ?, scheme_type = ?, updated_at = ? WHERE id_gateway = ?`,
				[
					item.name,
					item.description,
					item.quantity,
